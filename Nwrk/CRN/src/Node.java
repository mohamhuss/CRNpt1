...
public class Node implements NodeInterface {
    ...
    private int calculateDistance(byte[] hash1, byte[] hash2) {
        ...
        for (int i = 0; i < hash1.length; i++) {
            distance |= (hash1[i] ^ hash2[i]) & 0xFF; // XOR and accumulate
        }
        return distance;
    }
        ...
        if (message.startsWith("*") && message.contains("W") && message.contains("N:")) {
            ...
            if (parts.length >= 6 && parts[1].equals("W")) {
                try {
                    ...
                    addAddressKeyValuePair(pair); // Assuming this method adds to routing table
                    keyValueStore.put(key, value); // Store for quick access
