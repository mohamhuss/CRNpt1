...
// NodeInterface: Defines the contract for the Node class
...
public class Node implements NodeInterface {
    ...
    private int port;
    private volatile boolean poemFetched = false;
    ...
    //Public methods from node interface
    ...
    private boolean isPoemComplete() {
        // Assuming poem verses are keys like "D:Verse1", "D:Verse2", etc.
        // Adjust this logic based on the actual key pattern used by AzureLabTest
        int verseCount = 0;
        ...
        // Assume the test fetches a fixed number of verses, e.g., 4
        // You may need to adjust this number based on the test's requirements
        return verseCount >= 4; // Example threshold
    }
            ...
            for (int retry = 0; retry < MAX_RETRIES; retry++) {
                ...
                if (latch.await(REQUEST_TIMEOUT, TimeUnit.MILLISECONDS)) {
                    if (value[0] != null) {
                        ...
                        // Check if this is a poem verse and all are fetched
                        if (key.startsWith("D:") && isPoemComplete()) {
                            poemFetched = true; // Set flag when poem is complete
                        }
                        return value[0];
    ...
    private void startListenerThread() {
        listenerThread = new Thread(() -> {
            try {
                while (!poemFetched) { // Modified loop condition
    ...
    private void processMessage(DatagramPacket packet) {
        ...
        try {
            ...
            // Only treat 'S', 'F', 'X', 'D' as responses
            if (messageType == 'S' || messageType == 'F' || messageType == 'X' || messageType == 'D') {
    ...
    // Optional shutdown method (call externally if needed)
