...
// This class implements the `NodeInterface` and provides full functionality for a network node.
public class Node implements NodeInterface {
    ...
    // **ResponseCallback**
    // Interface for handling asynchronous responses to network requests.
    ...
    // **HashID**
    // Utility class for computing hash IDs (simplified placeholder).
    private static class HashID {
        static byte[] computeHashID(String input) throws Exception {
            return input.getBytes(StandardCharsets.UTF_8); // Placeholder implementation
        }
    }
    ...
    private byte[] padHash(byte[] hash, int targetLength) {
        ...
        return padded; // Remaining bytes are zero by default
    }
    ...
    public String read(String key) throws Exception {
        ...
        List<String> nodesToQuery = findClosestNodeNames(keyHash);
        Set<String> queriedNodes = new HashSet<>();

        while (!nodesToQuery.isEmpty()) {
            String nodeName = nodesToQuery.remove(0);
            if (queriedNodes.contains(nodeName)) continue;
            queriedNodes.add(nodeName);
            ...
            for (int attempt = 0; attempt < MAX_RETRIES; attempt++) {
                ...
                final List<String> nearestNodes = new ArrayList<>();
                final CountDownLatch latch = new CountDownLatch(1);

                String txid = generateTransactionID();
                String formattedKey = formatString(key).trim();
                ...
                ResponseCallback callback = new ResponseCallback() {
                    ...
                    public void onResponse(String response) {
                        ...
                        } else if (response.startsWith(txid + " O ")) {
                            String nodeList = response.substring(txid.length() + 2).trim();
                            String[] pairs = nodeList.split("\\s+0\\s+");
                            for (String pair : pairs) {
                                if (pair.contains("N:") && pair.contains(":")) {
                                    nearestNodes.add(pair.split("\\s+")[0]);
                                }
                            }
                        }
                        latch.countDown();
