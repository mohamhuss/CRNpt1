...
public class Node implements NodeInterface {
    ...
    private void processMessage(String message, DatagramPacket packet) throws Exception {
        String[] parts = message.split("\\s+", 3);
        if (parts.length < 2) {
            System.out.println("[DEBUG] Invalid message format: " + message);
            return;
        }
        String txid = parts[0];
        char command = parts[1].charAt(0);
        String args = parts.length > 2 ? parts[2] : "";
        System.out.println("[DEBUG] Parsed TXID: " + txid + ", Command: " + command + ", Args: " + args);
        ...
        // Handle requests based on command
        switch (command) {
            case 'G': // Name request
    ...
    private void handleNearestRequest(String txid, String args, InetAddress address, int port) throws Exception {
    ...
    private void handleKeyExistenceRequest(String txid, String args, InetAddress address, int port) throws Exception {
    ...
    private void handleReadRequest(String txid, String args, InetAddress address, int port) throws Exception {
        String key = parseString(args);
        if (key == null) return;
    ...
    private void handleWriteRequest(String txid, String args, InetAddress address, int port) throws Exception {
    ...
    private void handleCASRequest(String txid, String args, InetAddress address, int port) throws Exception {
    ...
    private void handleRelayRequest(String txid, String args, InetAddress address, int port) throws Exception {
    ...
    private List<String> findClosestNodeNames(byte[] keyHash) {
        List<AddressKeyValuePair> nearestPairs = findNearestNodes(keyHash, 3);
        List<String> nodeNames = new ArrayList<>();
        for (AddressKeyValuePair pair : nearestPairs) {
                ...
                ResponseCallback callback = new ResponseCallback() {
                    ...
                    public void onResponse(String response) {
                        if (response.startsWith(txid + " S ")) {
                            char result = response.charAt(6);
                            if (result == 'Y' && response.length() > 8) {
                                value[0] = parseString(response.substring(8));
                            }
                        }
                        latch.countDown();
                    }
