...
public class Node implements NodeInterface {
    ...
    private List<NodeAddress> queryNearestNodes(byte[] keyHash, NodeAddress node) throws Exception {
        ...
        ResponseCallback callback = new ResponseCallback() {
            ...
            public void onResponse(String response) {
                if (response.length() >= 5 && response.charAt(3) == 'O') {
                    ...
                    String[] parts = nodeList.split("(?<=\\d) (?=[0-9])"); // Split by space between key-value pairs
                    for (String part : parts) {
                        String nodeName = parseString(part);
                        if (nodeName != null && nodeName.startsWith("N:")) {
                            nodeResponses.add(nodeName);
                        }
                    }
                }
                latch.countDown();
            ...
            @Override
            public void onTimeout() {
                System.out.println("Timeout querying nearest nodes from " + node.name);
                latch.countDown();
            }
        };

        // Send request
        System.out.println("Querying nearest nodes from " + node.name + " for key hash " + bytesToHex(keyHash));
        pendingTransactions.put(txid, callback);
        ...
        // Convert responses to NodeAddress objects
    ...
    public String read(String key) throws Exception {
        ...
        // Compute key hash and get initial closest nodes
        byte[] keyHash = HashID.computeHashID(key);
        List<String> initialClosestNodes = findClosestNodeNames(keyHash);

        // Track queried nodes to avoid cycles and manage nodes to query
        Set<String> queriedNodes = new HashSet<>();
        Queue<NodeAddress> nodesToQuery = new LinkedList<>();

        // Populate initial queue with closest nodes
        ...
        while (!nodesToQuery.isEmpty() && attempts < maxAttempts) {
            ...
            if (queriedNodes.contains(node.name)) continue; // Skip already queried nodes
            queriedNodes.add(node.name);
            attempts++;

            // Retry logic for the current node
            for (int retry = 0; retry < MAX_RETRIES; retry++) {
                ...
                // Log the request
                System.out.println("Sending read request to " + node.name + " (attempt " + (attempts) + "/" + maxAttempts + ", retry " + (retry + 1) + "): " + readRequest);

                ResponseCallback callback = new ResponseCallback() {
                    @Override
                    public void onResponse(String response) {
                        System.out.println("Received response from " + node.name + ": " + response);
                    ...
                    @Override
                    public void onTimeout() {
                        System.out.println("Timeout for request to " + node.name + ": " + readRequest);
                        latch.countDown();
                    }
                };
                ...
                if (latch.await(REQUEST_TIMEOUT, TimeUnit.MILLISECONDS)) {
                    if (value[0] != null) {
                        // Key found, store locally and return
                        keyValueStore.put(key, value[0]);
                        System.out.println("Key " + key + " found at " + node.name + ": " + value[0]);
                        return value[0];
                    } else if (responseChar[0] == '?') {
                        // Node doesn’t have the key and isn’t closest; query its nearest nodes
                        System.out.println(node.name + " doesn’t have key " + key + " and isn’t closest, querying nearest nodes");
                    ...
                    // If responseChar is 'N', retry or move to next node
                } else {
                    System.out.println("No response from " + node.name + " after retry " + (retry + 1));
                }
            }
        }

        // Log failure if key not found
        System.out.println("Failed to find key: " + key + " after " + attempts + " attempts across " + queriedNodes.size() + " nodes");
        return null;
    }
