...
public class Node implements NodeInterface {
    ...
    private void processMessage(byte[] messageBytes, DatagramPacket packet) throws Exception {
        // Check if message is long enough: 2 bytes TXID + 1 byte space + 1 byte command
        ...
        // Verify space at position 2
        if (messageBytes[2] != 0x20) {
            System.out.println("[DEBUG] No space after transaction ID");
            return;
        }

        // Extract command (single byte at position 3)
        char command = (char) messageBytes[3];

        // Extract arguments (remaining bytes after command)
        String args = "";
        if (messageBytes.length > 4) {
        ...
        switch (command) {
            case 'N':
                ...
                System.out.println("[INFO] Node announcement: " + args);
                break;
            case 'W':
                // Handle write message
                System.out.println("[INFO] Write message: " + args);
                break;
            case 'R':
                // Handle read request
                System.out.println("[INFO] Read request: " + args);
                // Add logic to process read request and respond
                break;
            case 'G':
                // Handle get message
                System.out.println("[INFO] Get message: " + args);
                break;
            case 'I':
                // Handle info message
                System.out.println("[INFO] Info message: " + args);
                break;
            default:
    ...
    private List<NodeAddress> queryNearestNodes(byte[] keyHash, InetAddress address, int port) throws Exception {
    ...
    public String read(String key) throws Exception {
        ...
        while (!nodesToQuery.isEmpty()) {
            ...
            if (queriedNodes.contains(node.opisName)) continue;
            queriedNodes.add(node.opisName);
            ...
            for (int attempt = 0; attempt < MAX_RETRIES; attempt++) {
                ...
                if (latch.await(REQUEST_TIMEOUT, TimeUnit.MILLISECONDS)) {
                    if (value[0] != null) {
                        ...
                        List<NodeAddress> nearestNodes = queryNearestNodes(keyHash, node.address, node.port);
