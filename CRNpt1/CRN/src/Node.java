...
public class Node implements NodeInterface {
    ...
    public String read(String key) throws Exception {
        ...
        // Find the closest nodes to the key
        // For important keys like poem verses, we want to be thorough
        boolean isPoemVerse = key.startsWith("D:jabberwocky");
        int nodesToQuery = isPoemVerse ? 5 : 3;

        List<String> closestNodes = findClosestNodesAndUpdateIfNeeded(keyHashID, nodesToQuery);

        // Try to expand our network knowledge if we don't have many nodes
        if (closestNodes.size() < nodesToQuery && !closestNodes.isEmpty()) {
            expandNetworkKnowledge(closestNodes);
            // Try again with our expanded knowledge
            closestNodes = findClosestNodesAndUpdateIfNeeded(keyHashID, nodesToQuery);
        }
                ...
                if (response != null && response.length() > 4 && response.charAt(3) == 'S') {
                    ...
                    if (parts.length >= 2) {
                        ...
                        if (responseChar == 'Y' && parts.length >= 3) {
                            ...
                            // The node is closest but doesn't have the key
                            // If this node is truly closest, we should try nearest request to make sure
                            if (isPoemVerse) {
                                // For poems, try to find where the data actually is
