...
public class Node implements NodeInterface {
    ...
    public String read(String key) throws Exception {
        ...
        // First, try to find the node that has this key by calculating the hashID
        // and finding the closest nodes
    ...
    private String sendRequestAndWaitForResponse(String request, InetAddress address, int port) throws Exception {
        ...
        try {
            ...
            // Wait for the response with a timeout (shorter timeout for better responsiveness)
            return pendingRequest.future.get(10, TimeUnit.SECONDS);
        } catch (Exception e) {
            pendingRequests.remove(transactionId);
            throw e;
        }
    ...
    private List<String> findClosestNodesAndUpdateIfNeeded(byte[] targetHashID, int count) throws Exception {
        ...
        // If we don't have enough nodes, try to find more through nearest requests
        if (initialNodes.size() < count) {
            // Query an existing node for nearest nodes
            for (String nodeName : initialNodes) {
            ...
            if (updatedNodes.size() > initialNodes.size()) {
                return updatedNodes;
            }
        }
